...
private struct FakeAlgorithm: SortAlgorithm {
    ...
    var metadata: AlgorithmMetadata {
        AlgorithmMetadata(
            ...
            stable: true, timeComplexity: ComplexityBounds(best: "O(n)", average: "O(n^2)", worst: "O(n^2)"),
...
struct SortCoordinatorTests {
    ...
    func registeringAndUnregisteringActiveSessionTracksTheRightAlgorithm() {
        ...
        let session = SortSession(algorithm: FakeAlgorithm(), shuffle: FakeShuffle(), settings: makeFastSettings())
    ...
    func pendingActionAndShuffleOverrideAreConsumedExactlyOnce() async {
        ...
        guard case let .run(visualizerID, size) = coordinator.consumePendingAction(for: algorithm.id) else {
    ...
    func runAutomationSelectsTheAlgorithmAndAwaitsResolveCompletion() async {
        ...
        guard case let .automation(resolvedID) = coordinator.consumePendingAction(for: algorithm.id) else {
    ...
    func eachRunBumpsRunTokenEvenForTheSameAlgorithm() async {
        ...
        let firstRun = Task { await coordinator.runSort(algorithm: algorithm, visualizerID: nil, shuffleID: nil, size: nil) }
        await Task.yield()
        ...
        let secondRun = Task { await coordinator.runSort(algorithm: algorithm, visualizerID: nil, shuffleID: nil, size: nil) }
        await Task.yield()
...
private func sampleAutomation() -> Automation {
    Automation(
        id: AutomationID(rawValue: "coordinator-fake-automation"), displayName: "Fake", iconName: "fake",
