...
struct MetalRendererViewSwitchingTests {
    ...
    func switchingVisualizerMidSortRebuildsAndReseedsTheRenderer() throws {
        ...
        let tape = Tape(header: header, operations: [.setValue(0, 20), .setValue(1, 10), .setValue(2, 40), .setValue(3, 30)])
        ...
        coordinator.setUp(replay: replay, renderer: initialRenderer, view: view, visualizerID: VisualizerID(rawValue: "bargraph"))
    ...
    func switchingToTheSameVisualizerDoesNotRebuild() throws {
        ...
        let tape = Tape(header: header, operations: [.setValue(0, 1), .setValue(1, 2), .setValue(2, 3)])
        let replay = ReplayEngine(tape: tape)
        ...
        coordinator.setUp(replay: replay, renderer: initialRenderer, view: view, visualizerID: bargraphID)
        view.delegate = initialRenderer
