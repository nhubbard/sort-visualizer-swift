...
struct MetalShapeRendererBufferConsistencyTests {
  ...
  func rainbowMetalBufferMatchesFinalSortedValuesAtN256() async throws {
    ...
    replay.onOperationApplied = { operation, _ in
      ...
      renderer.apply(
        ...
        markers: markers(for: replay.frame))
    }
  ...
  func rainbowMetalBufferStaysCorrectAfterMidPlaybackResize() async throws {
    ...
    #expect(
      replay.stepIndex < replay.totalOperationCount, "test setup: must still be mid-tape here")
  ...
  func touchedSlotColorEasesInsteadOfSnapping() throws {
    ...
    renderer.reset(
      values: values, valueRange: 10...20, markers: [:], canvasSize: canvasSize, scale: 1)
