...
enum MetalRendererFactory {
  ...
  static func makeRenderer(
    ...
    case "disparitydots":
      MetalShapeRenderer<DisparityDotsMetalLayout>(device: device, sampleCount: sampleCount)
    case "hoopstack":
      MetalShapeRenderer<HoopStackMetalLayout>(device: device, sampleCount: sampleCount)
    case "scatterplot":
      MetalShapeRenderer<ScatterPlotMetalLayout>(device: device, sampleCount: sampleCount)
    case "spiraldots":
      MetalShapeRenderer<SpiralDotsMetalLayout>(device: device, sampleCount: sampleCount)
    case "wavedots":
      MetalShapeRenderer<WaveDotsMetalLayout>(device: device, sampleCount: sampleCount)
    case "colorcircle":
      MetalTriangleRenderer<ColorCircleMetalLayout>(device: device, sampleCount: sampleCount)
    case "disparitycircle":
      MetalTriangleRenderer<DisparityCircleMetalLayout>(device: device, sampleCount: sampleCount)
    case "spiral":
