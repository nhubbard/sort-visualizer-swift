...
struct ColorCircleMetalLayout: MetalTriangleLayout {
    ...
    private static func point(atAngle theta: Double, center: SIMD2<Double>, radius: Double) -> SIMD2<Double> {
    ...
    static func instance(
        ...
        let start = Self.point(atAngle: Self.angle(index - 1, count: count), center: center, radius: radius)
        ...
        let color = MetalShapeColor.marker(forIndex: index, in: markers) ?? MetalShapeColor.hueRamp(normalized)
...
struct DisparityCircleMetalLayout: MetalTriangleLayout {
    ...
    private static func point(forIndex index: Int, values: [Int], center: SIMD2<Double>, radius: Double, count: Int) -> SIMD2<Double> {
    ...
    static func instance(
        ...
        let start = Self.point(forIndex: previousIndex, values: values, center: center, radius: radius, count: count)
        let end = Self.point(forIndex: index, values: values, center: center, radius: radius, count: count)
        let normalized = MetalShapeColor.normalized(value: values[index], in: valueRange)
        let color = MetalShapeColor.marker(forIndex: index, in: markers) ?? MetalShapeColor.hueRamp(normalized)
        return MetalTriangleInstance(
            p0: SIMD2(Float(center.x), Float(center.y)),
...
struct SpiralMetalLayout: MetalTriangleLayout {
    ...
    private static func point(
        forIndex index: Int, values: [Int], valueRange: ClosedRange<Int>, center: SIMD2<Double>, radius: Double, count: Int
    ) -> SIMD2<Double> {
        let normalized = MetalShapeColor.normalized(value: values[index], in: valueRange)
    ...
    static func instance(
        ...
        let start = Self.point(
            forIndex: previousIndex, values: values, valueRange: valueRange, center: center, radius: radius, count: count)
        let end = Self.point(
            forIndex: index, values: values, valueRange: valueRange, center: center, radius: radius, count: count)
        let normalized = MetalShapeColor.normalized(value: values[index], in: valueRange)
        let color = MetalShapeColor.marker(forIndex: index, in: markers) ?? MetalShapeColor.hueRamp(normalized)
        return MetalTriangleInstance(
            p0: SIMD2(Float(center.x), Float(center.y)),
