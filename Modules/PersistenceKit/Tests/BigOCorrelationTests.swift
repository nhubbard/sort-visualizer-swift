...
struct BigOCorrelationTests {
    ...
    func rendersReferenceCurvesForRadixSortsExtraVariables() async throws {
        ...
        let summaries = try await service.fetchSummaries(algorithmID: AlgorithmID(rawValue: "msdradixsort"))
        let complexity = ComplexityBounds(best: "O(d \\times n)", average: "O(d \\times n)", worst: "O(d \\times n)")
    ...
    func bingoSortCurveReflectsRecordedUniqueValueRatio() async throws {
        ...
        let summaries = try await service.fetchSummaries(algorithmID: AlgorithmID(rawValue: "bingosort"))
        let complexity = ComplexityBounds(best: "O(n+m^2)", average: "O(n \\times m)", worst: "O(n \\times m)")
        let points = bigOChartPoints(for: summaries, timeComplexity: complexity)
        // Average and Worst resolve to the same shape here, so they merge into one series.
        let averageCase = points.filter { $0.series == "Average & Worst Case" }.sorted { $0.size < $1.size }
