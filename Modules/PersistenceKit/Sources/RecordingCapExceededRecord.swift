...
/// (see `SortSession.start(size:)`). Meant for the developer to review later, across every device
/// signed into the same iCloud account, to decide whether to narrow that algorithm's `sizeRange`
/// or fix it outright. Same CloudKit sync as `BigORecord`, but deliberately has no
/// `fetchSummaries`-style accessor — nothing in the app ever reads this back.
///
/// Every property has a default even though the inserting call always supplies real ones —
/// CloudKit-backed SwiftData models require every attribute to have a default (or be optional);
/// a model without one fails to sync at runtime, not at compile time.
@Model
