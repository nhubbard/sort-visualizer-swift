...
struct HoopStackVisualizerTests {
    ...
    func hoopSizeScalesWithValue() {
        ...
        guard case let .ellipse(_, _, highWidth, highHeight, _) = commands[0],
              case let .ellipse(_, _, lowWidth, lowHeight, _) = commands[1]
        else {
    ...
    func colorsReflectMarkerState() {
        ...
        guard case let .ellipse(_, _, _, _, unmarkedColor) = commands[0],
              case let .ellipse(_, _, _, _, secondaryColor) = commands[1],
              case let .ellipse(_, _, _, _, primaryColor) = commands[2]
        else {
            Issue.record("expected .ellipse")
            return
        }
