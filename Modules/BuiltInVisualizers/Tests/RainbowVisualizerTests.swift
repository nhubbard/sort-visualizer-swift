...
struct RainbowVisualizerTests {
    ...
    func emitsOneRectPerValueSizedByRange() {
        ...
        guard case let .rect(_, y0, w0, h0, _) = commands[0],
              case let .rect(_, _, _, _, _) = commands[1],
              case let .rect(_, y2, _, h2, _) = commands[2]
        else {
    ...
    func lowAndHighValuesGetDifferentHues() {
        ...
        guard case let .rect(_, _, _, _, colorLow) = commands[0],
              case let .rect(_, _, _, _, colorHigh) = commands[1]
        else {
