...
struct SineWaveVisualizerTests {
  ...
  func barsAreThinTicksNotTallBars() {
    let context = makeContext(
      values: [1, 4], valueRange: 1...4, canvasSize: CGSize(width: 100, height: 200))
    let commands = SineWaveVisualizer().draw(context)

    guard case .rect(_, _, _, let height, _) = commands[0] else {
      Issue.record("expected .rect")
      return
    }
  ...
  func valueAtZeroNormalizedSitsAtVerticalCenter() {
    ...
    guard case .rect(_, let y, _, let height, _) = commands[0] else {
      Issue.record("expected .rect")
      return
    }
  ...
  func differentNormalizedValuesProduceDifferentYPositions() {
    ...
    let context = makeContext(
      values: [0, 1], valueRange: 0...4, canvasSize: CGSize(width: 100, height: 40))
    let commands = SineWaveVisualizer().draw(context)

    guard case .rect(_, let y0, _, _, _) = commands[0],
      case .rect(_, let y1, _, _, _) = commands[1]
    else {
      Issue.record("expected .rect")
      return
    }
