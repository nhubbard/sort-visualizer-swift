...
struct DisparityChordsVisualizerTests {
    ...
    func firstChordConnectsItsAngleToItsValuesAngle() {
        ...
        guard case let .line(x1, y1, x2, y2, _, _) = commands[0] else {
            Issue.record("expected .line")
            return
        }
        let centerX = 100.0, centerY = 100.0
    ...
    func colorsReflectMarkerState() {
        ...
        guard case let .line(_, _, _, _, color0, _) = commands[0],
              case let .line(_, _, _, _, color1, _) = commands[1],
              case let .line(_, _, _, _, color2, _) = commands[2]
        else {
            Issue.record("expected .line")
            return
        }
    ...
    func degenerateCanvasSizeProducesNoCommands() {
        #expect(DisparityChordsVisualizer().draw(makeContext(values: [1, 2], canvasSize: .zero)).isEmpty)
    }
}
