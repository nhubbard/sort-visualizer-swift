...
struct DisparityCircleVisualizerTests {
  ...
  func emitsOneTriangleWedgePerValue() {
    ...
    for command in commands {
      guard case .polygon(let points, _) = command else {
  ...
  func disparityMatchesHandComputedValuesForReverseSortedInput() {
    ...
    for (index, value) in [4, 3, 2, 1].enumerated() {
      ...
      #expect(
        abs(disp - expected[index]) < 0.0001,
        "index \(index): expected \(expected[index]), got \(disp)")
    }
  }
  ...
  func sortedArrayProducesUniformRadiusPoints() {
    ...
    let radii = commands.compactMap { command -> Double? in
      guard case .polygon(let points, _) = command, points.count == 3 else { return nil }
  ...
  func colorsReflectMarkerState() {
    ...
    guard case .polygon(_, let color0) = commands[0],
      case .polygon(_, let color1) = commands[1],
      case .polygon(_, let color2) = commands[2]
    else {
      Issue.record("expected .polygon")
      return
    }
  ...
  func degenerateCanvasSizeProducesNoCommands() {
    #expect(
      DisparityCircleVisualizer().draw(makeContext(values: [1, 2], canvasSize: .zero)).isEmpty)
  }
}

extension SIMD2<Double> {
  fileprivate var length: Double { (x * x + y * y).squareRoot() }
}
