...
private func makeMetadata(best: String, average: String, worst: String, space: String) -> AlgorithmMetadata {
    AlgorithmMetadata(
        displayName: "Fake",
        category: .exchange,
        sizeRange: 1...1,
...
struct ComplexityRowTests {
    ...
    func alreadySpacedTimesStillRendersCorrectly() {
        let metadata = makeMetadata(best: "O(n)", average: "O(n * n!)", worst: "O(n * n!)", space: "O(1)")
    ...
    func logBecomesALatexCommand() {
        let metadata = makeMetadata(best: "O(n log n)", average: "O(n log n)", worst: "O(n log n)", space: "O(n)")
