...
struct GoldenCorpusTests {
  ...
  // decodecorpus`), run once offline at a fixed seed (`-s7 -n200 --max-content-size-log=14`) and
  // verified frame-by-frame against real `unzstd` before being committed. Exercises the space of
  // valid frame shapes (window sizes, single/multi-segment, checksum on/off, content-size field
  // widths, block splits) independent of anything this project's own encoder happens to produce.
