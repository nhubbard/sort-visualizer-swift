...
struct AmplitudeEnvelopeTests {
    ...
    func singleStepMovesTowardTargetNotAwayFromIt() {
        ...
        #expect(phase == .attack, "one step at a 0.1s time constant shouldn't already cross the 0.999 threshold")
    }
    ...
    func longerAttackDurationRisesMoreSlowlyThanShorterOne() {
        ...
        #expect(fast.gain > slow.gain, "a shorter attack time constant should reach a higher gain in the same steps")
    }
}
