  ...
  public func getFormat(token: CodeAttributes.Value) -> TextFormat {
    TextFormat.getBuilder(bg: getBgColor()).then { it in
      ...
      case .comment, .commentHashbang, .commentMultiline, .commentPreprocFile, .commentSingle,
        .commentSpecial:
        it.fg("#6272a4")
      case .error, .generic, .literal, .name, .other, .punctuation, .genericError, .genericPrompt,
        .genericStrong,
      ...
      case .genericHeading, .genericSubheading, .genericInserted:
        it.fg("#f8f8f2").bold()
      case .keyword, .operator, .commentPreproc, .keywordConstant, .keywordNamespace,
        .keywordPseudo,
      ...
      case .keywordDeclaration, .nameBuiltin, .nameLabel, .nameVariable:
        it.fg("#8be9fd").italic()
      case .number, .numberBin, .numberFloat, .numberHex, .numberInteger, .numberOct,
        .numberIntegerLong:
        it.fg("#ffb86c")
      case .string, .stringAffix, .stringBacktick, .stringChar, .stringDelimiter, .stringDoc,
        .stringDouble,
