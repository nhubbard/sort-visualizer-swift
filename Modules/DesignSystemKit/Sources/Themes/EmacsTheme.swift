...
public struct EmacsTheme: CodeTheme {
  ...
  public func getFormat(token: CodeAttributes.Value) -> TextFormat {
    TextFormat.getBuilder(bg: getBgColor()).then { it in
      ...
      case .keywordType:
        it.fg("#0b0").bold()
      case .string, .nameAttribute, .stringAffix, .stringBacktick, .stringChar, .stringDelimiter,
        .stringDouble,
      ...
      case .stringInterpol:
        it.fg("#b68").bold()
      case .stringSymbol, .nameVariableClass, .nameVariableGlobal, .nameVariableInstance,
        .nameVariableMagic:
        it.fg("#b8860b")
      default:
