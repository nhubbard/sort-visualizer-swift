...
public struct PygmentsTheme: CodeTheme {
  ...
  public func getFormat(token: CodeAttributes.Value) -> TextFormat {
    TextFormat.getBuilder(bg: hl).then { it in
      ...
      case .error:
        it.bg(err)
      case .commentHashbang, .commentMultiline, .commentPreprocFile, .commentSingle,
        .commentSpecial:
        it.fg(c).italic()
      case .commentPreproc:
        it.fg(cp)
      ...
      case .genericTraceback:
        it.fg(gt)
      case .keyword, .keywordConstant, .keywordDeclaration, .keywordNamespace, .keywordReserved,
        .nameTag:
        it.fg(k).bold()
      ...
      case .operator, .number, .numberBin, .numberFloat, .numberHex, .numberInteger, .numberOct,
        .numberIntegerLong:
        it.fg(o)
      case .string, .stringAffix, .stringBacktick, .stringChar, .stringDelimiter, .stringDouble,
        .stringHeredoc,
        .stringSingle:
        it.fg(s)
      case .nameAttribute:
