...
public struct ArduinoTheme: CodeTheme {
  ...
  public func getFormat(token: CodeAttributes.Value) -> TextFormat {
    TextFormat.getBuilder(bg: .white).then { it in
      ...
      case .error:
        it.fg(error)
      case .keyword, .operator, .commentPreproc, .keywordDeclaration, .keywordNamespace,
        .nameBuiltin, .nameOther,
        .operatorWord, .nameBuiltinPseudo:
        it.fg(keyword)
      case .name, .nameAttribute, .nameClass, .nameConstant, .nameDecorator, .nameEntity,
        .nameException,
