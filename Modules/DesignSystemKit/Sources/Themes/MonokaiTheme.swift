...
private let operatorColor = Color(fromHex: "#F92672")!
...
public struct MonokaiTheme: CodeTheme {
  public init() {}
  public func getBgColor() -> Color { bgColor }
  public func getFormat(token: CodeAttributes.Value) -> TextFormat {
    TextFormat.getBuilder(bg: bgColor).then { it in
      switch token {
      case .comment, .commentHashbang, .commentMultiline, .commentPreproc, .commentPreprocFile,
        .commentSingle,
        .commentSpecial, .genericSubheading:
        it.fg(comment)
      case .error:
        it.fg(errorFg).bg(errorBg)
      case .escape, .generic, .name, .other, .punctuation, .genericError, .genericHeading,
        .genericTraceback,
        .nameBuiltin, .nameEntity, .nameLabel, .nameNamespace, .nameProperty, .nameVariable,
        .whitespace,
        .nameBuiltinPseudo, .nameVariableClass, .nameVariableGlobal, .nameVariableInstance,
        .nameVariableMagic:
        it.fg(text)
      case .keyword, .genericOutput, .keywordConstant, .keywordNamespace, .keywordDeclaration,
        .keywordPseudo,
        .keywordReserved, .keywordType, .nameConstant:
        it.fg(keyword)
      case .literal, .number, .numberBin, .numberFloat, .numberHex, .numberInteger, .numberOct,
        .stringEscape,
        .numberIntegerLong:
        it.fg(number)
      ...
      case .genericStrong:
        it.fg(text).bold()
      case .genericInserted, .nameAttribute, .nameClass, .nameDecorator, .nameException,
        .nameFunction,
        .nameOther, .nameFunctionMagic:
        it.fg(name)
      case .genericPrompt:
        it.fg(operatorColor).bold()
      case .literalDate, .string, .stringAffix, .stringBacktick, .stringChar, .stringDelimiter,
        .stringDoc,
