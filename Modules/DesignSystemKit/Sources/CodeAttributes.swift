...
/// Ported verbatim from `Legacy/Shared/Views/Code/CodeAttributes.swift` — a custom
/// `AttributedString` key so `AttributedString(markdown:including:)` can parse the
/// `^[text](code: 'Token.X')` syntax the bundled `*.md` code samples already use (Pygments-style
/// token names), then `CodeTheme.getFormat(token:)` maps each token to a `TextFormat`.
public enum CodeAttributes: CodableAttributedStringKey, MarkdownDecodableAttributedStringKey {
...
extension AttributeScopes {
    public struct SortSymphonyAttributes: AttributeScope {
        public let code: CodeAttributes
    }

    public var sortSymphonyApp: SortSymphonyAttributes.Type { SortSymphonyAttributes.self }
}
