Iterative 3-Smooth Comb Sort is a [Shellsort](https://en.wikipedia.org/wiki/Shellsort) variant that uses Pratt's sequence of 3-smooth gaps. Shellsort generalizes insertion sort by first comparing and exchanging
...
algorithm, and one of the most studied choices is V. Pratt's sequence: every gap of the form 2ᵃ × 3ᵇ below the
...
The algorithm computes this gap sequence through a different mechanism than Classic 3-Smooth Comb Sort. The
...
Iterative 3-Smooth Comb Sort instead generates the gaps by construction: two nested loops range directly over the two
exponents themselves. The outer loop counts an exponent k down from the largest power of two not exceeding
length − 1; for each k, an inner loop counts an exponent j down from the largest value such that 2ᵏ × 3ʲ still
stays below length. Every (k, j) pair the loops visit produces exactly one gap, 2ᵏ × 3ʲ, and the array receives
