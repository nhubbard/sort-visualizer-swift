...
up to minimum + n − 1 exactly once, in some order. Under that condition, a value does not need to
...
position finally holds the value it is supposed to hold. Every such swap places at least one value
...
time linear in the number of elements, substantially faster than any comparison-based sort, but
