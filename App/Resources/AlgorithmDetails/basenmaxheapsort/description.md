...
sift-down instead scans all `N` children of a node (as many as still fall within the heap's current bound) to find the
largest among them, and only then compares that winner against the node itself before swapping and recursing further
down. Widening the fan-out this way shortens the heap's height from roughly `log₂ n` to `log₄ n` for `N` = 4, so fewer
levels need to be sifted through to reach a leaf, at the cost of each individual sift-down step doing more comparisons
per level to find the largest of up to four children instead of the larger of two.

Because heap height shrinks roughly in proportion to `log N`, while the per-node work at each level grows by close to
the same factor, the two effects offset each other in the asymptotic bound: Base-N Max Heap Sort remains `O(n log n)` in
