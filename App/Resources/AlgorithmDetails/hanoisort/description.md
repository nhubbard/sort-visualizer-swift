...
recursively: a genuine Hanoi tower move of height k always takes exactly 2ᵏ − 1 individual disk
moves, and since the height of the tower being moved is not known ahead of time, the algorithm just
keeps making legal one-at-a-time moves between pegs until a stopping condition, checked after
every move, reports that the move it is replaying has finished.
...
distinct values. With that adjustment in place, the overall structure is recognizably an
