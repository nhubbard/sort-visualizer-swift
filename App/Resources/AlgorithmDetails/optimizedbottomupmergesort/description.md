...
already compares equal to it. This preserves the stability of the sort. Once every block is
...
Whenever a pass's final pair of runs does not divide evenly, the array length is not a clean
...
it (a merge tolerates its second run being shorter than its first), and a leftover that
