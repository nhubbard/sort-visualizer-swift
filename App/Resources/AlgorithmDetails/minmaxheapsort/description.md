...
[min-max heap](https://en.wikipedia.org/wiki/Min-max_heap) (Atkinson, Sack, Santoro and Strothotte, 1986), a structure
...
to everything beneath it. Because the root sits at depth zero, it is always the minimum. Because the maximum can never
live on a "less than or equal" level, it is forced down to depth one, meaning it is always sitting at one of the root's
...
max) as the node itself, so they are exactly where a same-direction violation would show up first. Whichever of those
up to six descendants is most extreme becomes the candidate: if it is a direct child, one comparison and at most one
swap settles things. If it is a grandchild, the value drops two levels in a single swap, and then the intermediate
