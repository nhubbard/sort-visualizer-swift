...
Dual-Pivot Quick Sort is a variant of [Quick Sort](https://en.wikipedia.org/wiki/Quicksort) that partitions around **two
** pivot elements instead of one. Where classic Quick Sort splits a range into two regions each pass — "less than the
pivot" and "greater than the pivot" — Dual-Pivot Quick Sort splits it into three: elements less than the smaller pivot,
...
amount of extra memory per partitioning step, plus whatever stack space the recursion itself consumes. It is not
a [stable sort](https://en.wikipedia.org/wiki/Sorting_algorithm#Stability) — equal elements can be reordered relative to
one another as they cross partition boundaries. Its average-case running time matches classic Quick Sort's O(n log n),
but with fewer average swaps and better cache behavior in practice, which is what made it an attractive default for
