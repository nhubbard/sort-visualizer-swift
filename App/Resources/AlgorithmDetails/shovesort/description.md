Shove Sort scans an array left to right looking for an adjacent pair that is out of order. When it
finds one, it does not simply swap the two elements or shift the offender into its correct spot,
it "shoves" it all the way to the end of the range instead, via a chain of adjacent swaps that
walks from the offending position to the last index. That chain performs a one-step
...
After a shove, the scan backs up by one position (unless it is already at the start of the range)
