Rotate MSD Radix Sort is a most-significant-digit (MSD) radix sort that never allocates a bucket
...
machinery used by its least-significant-digit (LSD) counterpart: to group a range by its current digit, it treats each
...
there is nothing left to compare within that bucket.
