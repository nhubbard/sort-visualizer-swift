...
adjacent pair in a fixed order, it repeatedly picks a *random* adjacent pair and swaps the two elements if they are out
of order. Every accepted swap strictly fixes one inversion, exactly like ordinary Bubble Sort, but nothing guarantees
*which* inversion gets fixed next or how many draws land on a pair that is already in order.
