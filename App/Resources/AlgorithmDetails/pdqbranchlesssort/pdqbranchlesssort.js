...
function swapOffsets(arr, first, last, leftOffsets, leftPos, rightOffsets, rightPos, num, useSwaps) {
    if (useSwaps) {
        for (let i = 0; i < num; i++) {
...
function partRightBranchless(arr, begin, end, leftOffsets, rightOffsets) {
    ...
    let leftNum = 0, rightNum = 0, leftStart = 0, rightStart = 0;
    ...
    const unknownLeft = (last - first) - ((rightNum !== 0 || leftNum !== 0) ? BLOCK_SIZE : 0);
    if (rightNum !== 0) {
...
function pdqLoop(arr, begin, end, badAllowed, leftOffsets, rightOffsets) {
    ...
    while (true) {
        ...
        const [pivotPos, alreadyParted] = partRightBranchless(arr, begin, end, leftOffsets, rightOffsets);
