...
algorithm that, like its least-significant-digit (LSD) counterpart, distributes elements into buckets according to their digits rather than
comparing them directly. Where LSD radix sort processes digits from the last to the first in a
...
MSD radix sort is also well suited to variable-length keys, such as strings, since a bucket with no remaining
