...
being touched, while a descending pair marks the start of a run that is tracked until it turns back
...
Assuming the array was not just one big reversed run, the second phase is a bottom-up merge that
...
blocks that is already fully in order relative to each other is detected with a single comparison of
...
unrelated value the way a partitioning or exchange sort would. It is a stable sort. The
