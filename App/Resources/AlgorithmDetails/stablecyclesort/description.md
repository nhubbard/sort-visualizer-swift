...
order of equal elements, a property ordinary Cycle Sort does not have. Cycle Sort is known for making the theoretical
...
value currently held at the start of a cycle, it counts how many elements are strictly smaller (as ordinary Cycle Sort
does) and how many equal, not-yet-flagged duplicates lie between the start of the current cycle and the
position that started it. That second count lets each occurrence of a repeated value be placed after every other
occurrence of the same value that appeared earlier in the original array and has not been resolved yet, while skipping
...
exchange for stability. The comparison-count and rotation structure otherwise follows ordinary Cycle Sort, so
Stable Cycle Sort retains the same O(n) best case, O(n²) average and worst-case running time, and it remains an
