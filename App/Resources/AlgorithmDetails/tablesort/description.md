Table Sort is a median-of-three quicksort dressed up so that it never moves the array
...
happened during partitioning. For each position whose table entry does not already point to
...
order of equal elements and is a stable sort, despite being built on the same
partitioning shape as an unstable in-place quicksort. The underlying algorithm is still
fundamentally a comparison-and-swap partitioning scheme; the table of indices is
