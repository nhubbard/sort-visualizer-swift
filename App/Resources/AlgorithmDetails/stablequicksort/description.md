...
sort stable by construction. The cost
...
swap-based partition needs, in addition to the usual O(log n) recursive call stack.

Its running time follows the same pattern as any single-pivot Quick Sort that always chooses the
first element as its pivot: O(n log n) on average, but O(n²) in the worst case on adversarial
