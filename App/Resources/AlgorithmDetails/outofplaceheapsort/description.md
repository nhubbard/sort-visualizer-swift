...
accumulates. Instead of shrinking the heap by one slot and swapping the extracted maximum out to
...
being sifted, all the way down to a leaf. Only once that leaf is reached does the search climb back
...
only the climb needs to check the value being placed.
