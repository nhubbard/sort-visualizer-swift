...
define the value span it is working with. That span is divided into a number of equal-width
...
Because range-normalized bucketing spreads values out but does not guarantee that every bucket ends
...
over each bucket's own slice of the array. This backstop is what finishes the sort: the
