...
positive integer is 3-smooth if its only prime factors are 2 and 3; that is, it can be written as 2ᵃ × 3ᵇ for some
non-negative integers a and b, a family that includes 1, 2, 3, 4, 6, 8, 9, 12, 16, 18, 24, and so on. The algorithm
walks every candidate gap from length − 1 down to 1, discards any gap that is not of this form, and for each gap that
...
Unlike ordinary Comb Sort or a general Shellsort, the algorithm performs only *one* pass at each
surviving gap. It does not repeat a gap until no values move or compute an adaptive shrink factor
...
being sorted, Classic 3-Smooth Comb Sort is a sorting network rather than a heuristic.
