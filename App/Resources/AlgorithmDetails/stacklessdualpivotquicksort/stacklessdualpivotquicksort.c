...
int partition(int arr[], int start, int end, int scratch) {
  ...
  if (arr[m1] > arr[m2]) {
    swapInts(&arr[m1], &arr[start]);
    end--;
    swapInts(&arr[m2], &arr[end]);
  } else {
    swapInts(&arr[m2], &arr[start]);
    end--;
    swapInts(&arr[m1], &arr[end]);
  }
 ...
 * segment at a time, shrinking and partitioning it down to INSERTION_THRESHOLD
...
void quickSort(int arr[], int start, int end) {
  ...
  while (1) {
    ...
    reuseMedianCandidates = 1;
    while (a < segmentEnd && arr[a - 1] == arr[a]) {
      reuseMedianCandidates = 0;
      a++;
    }
    if (a == segmentEnd) {
      reuseMedianCandidates = 1;
