...
or **co-rank** search. An ordinary binary search identifies where a particular value belongs in the other run. The
co-rank search instead determines how many elements from each run, taken from the front of one and the back of the
other, constitute the c smallest elements of the combined run. To select the c smallest
