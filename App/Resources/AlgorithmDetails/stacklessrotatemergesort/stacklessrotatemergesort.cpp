...
void sort(int arr[], int n) {
  rotatePartitionMergeSort(arr, n);
}
...
void partitionMerge(int arr[], int a, int m, int b, int c) {
  ...
  if (lenB < lenA) {
    int cc = (lenA + lenB) - c;
    int r1 = std::max(0, cc - lenA);
    int r2 = std::min(cc, lenB);
    while (r1 < r2) {
      int ml = r1 + (r2 - r1) / 2;
    ...
    int r1 = std::max(0, c - lenB);
    int r2 = std::min(c, lenA);
    while (r1 < r2) {
      int ml = r1 + (r2 - r1) / 2;
  ...
  for (int j = 2; j < n; j *= 2) {
    ...
    for (int k = j / 2; k > 1; k /= 2) {
      ...
      while (seamStart + k < b1) {
        int seamEnd = std::min(seamStart + 2 * k, n);
