...
public class grailsort {
  ...
  public static Pair smartMergeWithoutBuffer(int[] arr, int pos, int leftOverLen, int leftOverFrag, int regBlockLen) {
    if (regBlockLen == 0) return new Pair(leftOverLen, leftOverFrag);
    ...
    if (len1 != 0 && (compareValues(arr[pos + len1 - 1], arr[pos + len1]) - typeFrag) >= 0) {
      while (len1 != 0) {
        ...
        if (len2 == 0) return new Pair(len1, leftOverFrag);
        do {
          pos++;
          len1--;
  ...
  public static Pair smartMergeWithBuffer(int[] arr, int pos, int leftOverLen, int leftOverFrag, int blockLen) {
    int dist = -blockLen;
    int left = 0;
  ...
  public static void combineBlocks(int[] arr, int keyPos, int pos, int len, int buildLen, int regBlockLen, boolean havebuf) {
