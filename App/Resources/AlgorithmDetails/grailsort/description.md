...
to) determines the direction of a tie-breaking comparison. Consequently, whenever two
...
variant earns the "stable" name where a leaner sibling does not.
...
full stability without merge sort's usual O(n) space cost. It is also the reference algorithm behind
