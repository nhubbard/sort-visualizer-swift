...
void mergeLeft(int arr[], int pos, int leftLen, int rightLen, int dist) {
  ...
  while (right < rightLen) {
    if (left == leftLen || arr[pos + left] > arr[pos + right]) {
      swapAt(arr, pos + dist, pos + right); dist++; right++;
    } else {
      swapAt(arr, pos + dist, pos + left); dist++; left++;
    }
  }
...
void mergeRight(int arr[], int pos, int leftLen, int rightLen, int dist) {
  ...
  while (left >= 0) {
    if (right < leftLen || arr[pos + left] > arr[pos + right]) {
      swapAt(arr, pos + mergedPos, pos + left); mergedPos--; left--;
    } else {
      swapAt(arr, pos + mergedPos, pos + right); mergedPos--; right--;
    }
  }
...
/* Returns the new leftover length; writes the new leftover fragment back through fragOut. */
int smartMergeWithoutBuffer(int arr[], int pos, int leftOverLen, int leftOverFrag, int regBlockLen, int *fragOut) {
  if (regBlockLen == 0) {
...
/* Returns the new leftover length; writes the new leftover fragment back through fragOut. */
int smartMergeWithBuffer(int arr[], int pos, int leftOverLen, int leftOverFrag, int blockLen, int *fragOut) {
  int dist = -blockLen;
  int left = 0;
  ...
  if (left < leftEnd) {
    ...
    while (left < leftEnd) {
      leftEnd--; rightEnd--;
      swapAt(arr, pos + leftEnd, pos + rightEnd);
    }
  } else {
...
void mergeBuffersLeft(int arr[], int keysPos, int midkey, int pos, int blockCount, int blockLen,
  ...
  for (int keyIndex = 1; keyIndex < blockCount; keyIndex++) {
    ...
    if (nextFrag == leftOverFrag) {
      ...
      int newFrag;
      int newLen;
      if (havebuf) newLen = smartMergeWithBuffer(arr, pos + restToProcess, leftOverLen, leftOverFrag, blockLen, &newFrag);
      else newLen = smartMergeWithoutBuffer(arr, pos + restToProcess, leftOverLen, leftOverFrag, blockLen, &newFrag);
      leftOverLen = newLen;
...
void buildBlocks(int arr[], int pos, int len, int buildLen) {
  for (int dist = 1; dist < len; dist += 2) {
    ...
    swapAt(arr, pos + dist - 3, pos + dist - 1 + extraDist);
    swapAt(arr, pos + dist - 2, pos + dist - extraDist);
  }
  if (len % 2 == 1) swapAt(arr, pos + len - 1, pos + len - 3);
  pos -= 2;
  int part = 2;
  while (part < buildLen) {
    int left = 0;
...
void combineBlocks(int arr[], int keyPos, int pos, int len, int buildLen, int regBlockLen, bool havebuf) {
  ...
  for (int i = 0; i <= combineLen; i++) {
    ...
    for (int index = 1; index < blockCount; index++) {
      ...
      if (leftIndex != index - 1) {
        ...
        swapAt(arr, keyPos + (index - 1), keyPos + leftIndex);
  ...
  if (havebuf) {
    while (len > 0) {
      len--;
      swapAt(arr, pos + len, pos + len - regBlockLen);
    }
  }
}
...
void lazyStableSort(int arr[], int pos, int len) {
  for (int dist = 1; dist < len; dist += 2) {
    if (arr[pos + dist - 1] > arr[pos + dist]) swapAt(arr, pos + dist - 1, pos + dist);
  }
  int part = 2;
  while (part < len) {
    int left = 0;
