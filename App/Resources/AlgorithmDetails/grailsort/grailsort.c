...
void multiSwap(int arr[], int a, int b, int count) {
  for (int i = 0; i < count; i++) swap(&arr[a + i], &arr[b + i]);
}
...
void mergeLeft(int arr[], int pos, int leftLen, int rightLen, int dist) {
  ...
  while (right < rightLen) {
    if (left == leftLen || arr[pos + left] > arr[pos + right]) {
      swap(&arr[pos + dist], &arr[pos + right]); dist++; right++;
    } else {
      swap(&arr[pos + dist], &arr[pos + left]); dist++; left++;
    }
  }
...
void mergeRight(int arr[], int pos, int leftLen, int rightLen, int dist) {
  ...
  while (left >= 0) {
    if (right < leftLen || arr[pos + left] > arr[pos + right]) {
      swap(&arr[pos + mergedPos], &arr[pos + left]); mergedPos--; left--;
    } else {
      swap(&arr[pos + mergedPos], &arr[pos + right]); mergedPos--; right--;
    }
  }
...
void mergeBuffersLeft(int arr[], int keysPos, int midkey, int pos, int blockCount, int blockLen,
                       int havebuf, int aBlockCount, int lastLen) {
  if (blockCount == 0) {
...
void buildBlocks(int arr[], int pos, int len, int buildLen) {
  ...
  if (len % 2 == 1) swap(&arr[pos + len - 1], &arr[pos + len - 3]);
  pos -= 2;
  int part = 2;
  while (part < buildLen) {
    int left = 0;
...
void combineBlocks(int arr[], int keyPos, int pos, int len, int buildLen, int regBlockLen, int havebuf) {
...
void lazyStableSort(int arr[], int pos, int len) {
  for (int dist = 1; dist < len; dist += 2) {
    if (arr[pos + dist - 1] > arr[pos + dist]) swap(&arr[pos + dist - 1], &arr[pos + dist]);
  }
  int part = 2;
  while (part < len) {
    int left = 0;
