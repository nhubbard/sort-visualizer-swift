...
specific left-strict and right-otherwise comparison rule chosen here, not an automatic feature of "build a binary search tree and read it
back".
...
input. An unfavorable arrival order, particularly an input that is already sorted (ascending or descending), makes every
...
sort. Unlike some other insertion-family sorts, none of this happens in place. The tree's left and right child pointers and
