Left–Right (LR) Quick Sort is a quicksort variant that uses the **Hoare partition scheme** and selects the middle element of each range as its pivot. The Hoare scheme is the original partitioning method C. A. R. Hoare described
...
Because the two pointers only ever swap when they have each independently found something out of place
...
rather than around a single clean split index the way Lomuto's scheme produces.
...
a median-of-three sample, a specifically constructed adversarial input can still drive this variant to its own O(n²)
worst case, exactly as with any quicksort that does not guard against adversarial pivot selection.
