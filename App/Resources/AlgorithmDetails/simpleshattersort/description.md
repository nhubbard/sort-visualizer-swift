...
Repeating the bucketing pass does not change what the algorithm is fundamentally capable of, a
...
last bucketing pass and the insertion-sort backstop run, there is less local disorder left to clean
up. It is a performance refinement layered on top of the same core distribution-sort idea, not a
correctness requirement, the insertion sort at the end is still what guarantees every
bucket ends up sorted.

Like Shatter Sort, Simple Shatter Sort is categorized as a distribution sort and is stable: every
bucketing pass places an element based only on its own value relative to the current minimum-to-maximum span,
