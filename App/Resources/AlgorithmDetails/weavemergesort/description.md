...
deck of cards is riffled together without anyone looking at the faces. Only after this position-blind interleave does
the algorithm look at values at all, running a single
ordinary [insertion sort](https://en.wikipedia.org/wiki/Insertion_sort) pass over the whole woven range to put it back
into actual sorted order.
...
efficiency, running in Θ(n²) time in the best, average, and worst cases alike, rather than the Θ(n log n) its recursive
shape might otherwise suggest.

The insertion-sort pass has one more consequence beyond speed: its shift condition slides a newly-placed element
leftward past every element it meets that is *less than or equal to* it, not just the ones strictly greater. That
non-strict tie-handling means two equal-valued elements can cross paths during the pass, so an element encountered later
in the woven sequence can end up ahead of an equal element that was already resting further to the left. Because this
same reordering-under-ties can also occur at every deeper level of the recursion, Weave Merge Sort is not
