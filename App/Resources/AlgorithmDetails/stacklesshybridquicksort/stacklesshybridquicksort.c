...
void medianOfThree(int arr[], int start, int end) {
  ...
  if (arr[start] > arr[mid]) {
    swapInts(&arr[start], &arr[mid]);
  }
  if (arr[mid] > arr[end - 1]) {
    swapInts(&arr[mid], &arr[end - 1]);
    if (arr[start] > arr[mid]) {
      return;
    }
  }
 ...
 * INSERTION_THRESHOLD elements, finishing with binaryInsertionSort, then
...
void quickSort(int arr[], int start, int end) {
  ...
  while (1) {
    while (segmentEnd - a > INSERTION_THRESHOLD) {
      ...
      swapInts(&arr[pivotIndex], &arr[tail]);
