...
the left subtree if it is smaller or the right subtree otherwise, until an empty spot is found. Once
...
in the general algorithm, but it is not an arbitrary one: consistently sending equal elements to one
