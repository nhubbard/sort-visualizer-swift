Iterative Pairwise Sort implements Parberry's pairwise [sorting network](https://en.wikipedia.org/wiki/Sorting_network),
a comparator-based sorting algorithm whose sequence of compare-and-swap operations is fixed in advance and does not depend on the input data. The same wires fire
...
it sorts every sequence of 0s and 1s correctly, the network can be designed and verified purely in terms of bits.
