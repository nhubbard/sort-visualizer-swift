Rotate LSD Radix Sort is an in-place least-significant-digit (LSD) radix sort that redistributes elements by rotation.
...
LSD radix sort visits digit positions starting from the ones place and working toward the
...
per-digit cost and constant auxiliary space, which is a design goal of the variant: it is an in-place LSD
