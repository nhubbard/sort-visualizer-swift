...
back up, now comparing the real value against each ancestor in turn, to find the exact level where it belongs,
and it is the climb, not the descent, that finally moves anything.
...
is expensive, comparing by a function call rather than a raw integer check, which is also why it is framed as an
