...
def quad_sort_range(arr, start, length):
    ...
    # instead. Allocates its own scratch buffer each call.
    if length < 16:
        tail_swap(arr, start, length)
    elif length < 256:
        if not quad_swap(arr, start, length):
            buffer = [0] * 128
...
def quad_sort_range_using(arr, swap_buf, start, length):
    # Same dispatch as quad_sort_range, but merges into `swap_buf` -- a caller-supplied scratch
    # buffer -- instead of allocating a fresh one. FluxSort uses this to reuse one top-level
    # scratch buffer across every depth of its own recursive partition.
    if length < 16:
        tail_swap(arr, start, length)
    elif length < 256:
        if not quad_swap(arr, start, length):
            tail_merge(arr, swap_buf, start, length, 16)
    else:
        if not quad_swap(arr, start, length):
            quad_merge(arr, swap_buf, start, length, 16)
...
def flux_analyze(arr, nmemb):
    ...
    # was fully handled without partitioning: already sorted, fully reverse-sorted (one reversal
    # away from sorted), or mostly-sorted-or-reversed enough (balance within 1/6 of either end)
    # that a plain quad_sort_range wins outright. Returns True only when real partitioning in
    # flux_partition is worthwhile.
    balance = 0
    pta = 0
    cnt = nmemb
    while True:
    ...
    if balance <= nmemb // 6 or balance >= nmemb // 6 * 5:
...
def main_gt(arr, swap_buf, main_is_swap, a, b):
    ...
    if main_is_swap:
        return swap_buf[a] > swap_buf[b]
    return arr[a] > arr[b]


def median_of_three(arr, swap_buf, main_is_swap, v0, v1, v2):
    # Median-of-3 index tournament.
    val = 1 if main_gt(arr, swap_buf, main_is_swap, v0, v1) else 0
    t0 = val
    t1 = val ^ 1

    val = 1 if main_gt(arr, swap_buf, main_is_swap, v0, v2) else 0
    t0 += val
    if t0 == 1:
        return v0

    val = 1 if main_gt(arr, swap_buf, main_is_swap, v1, v2) else 0
    t1 += val
    return v1 if t1 == 1 else v2
...
def median_of_five(arr, swap_buf, main_is_swap, v0, v1, v2, v3, v4):
    ...
    val = 1 if main_gt(arr, swap_buf, main_is_swap, v0, v3) else 0
    t0 += val
    t3 = val ^ 1

    val = 1 if main_gt(arr, swap_buf, main_is_swap, v0, v4) else 0
    t0 += val
    ...
    val = 1 if main_gt(arr, swap_buf, main_is_swap, v1, v3) else 0
    t1 += val
    t3 += val ^ 1

    val = 1 if main_gt(arr, swap_buf, main_is_swap, v1, v4) else 0
    t1 += val

    if t1 == 2:
        return v1

    val = 1 if main_gt(arr, swap_buf, main_is_swap, v2, v3) else 0
    t2 += val
    t3 += val ^ 1

    val = 1 if main_gt(arr, swap_buf, main_is_swap, v2, v4) else 0
    t2 += val

    if t2 == 2:
        return v2

    val = 1 if main_gt(arr, swap_buf, main_is_swap, v3, v4) else 0
    t3 += val

    return v3 if t3 == 2 else v4


def median_of_nine(arr, swap_buf, main_is_swap, ptx, nmemb):
    # Picks a pivot from 9 evenly-spaced samples via 3 median-of-3s feeding one more -- used
    # when the partition being pivoted is at most 1024 elements.
    div = nmemb // 16
    v0 = median_of_three(
        arr, swap_buf, main_is_swap, ptx + div * 2, ptx + div * 1, ptx + div * 4
    )
    v1 = median_of_three(
        arr, swap_buf, main_is_swap, ptx + div * 8, ptx + div * 6, ptx + div * 10
    )
    v2 = median_of_three(
        arr, swap_buf, main_is_swap, ptx + div * 14, ptx + div * 12, ptx + div * 15
    )
    return median_of_three(arr, swap_buf, main_is_swap, v0, v1, v2)


def median_of_fifteen(arr, swap_buf, main_is_swap, ptx, nmemb):
    # Picks a pivot from 15 evenly-spaced samples via 5 median-of-3s feeding one median-of-5 --
    # used once the partition being pivoted exceeds 1024 elements.
    div = nmemb // 16
    v0 = median_of_three(
        arr, swap_buf, main_is_swap, ptx + div * 2, ptx + div * 1, ptx + div * 3
    )
    v1 = median_of_three(
        arr, swap_buf, main_is_swap, ptx + div * 5, ptx + div * 4, ptx + div * 6
    )
    v2 = median_of_three(
        arr, swap_buf, main_is_swap, ptx + div * 8, ptx + div * 7, ptx + div * 9
    )
    v3 = median_of_three(
        arr, swap_buf, main_is_swap, ptx + div * 11, ptx + div * 10, ptx + div * 12
    )
    v4 = median_of_three(
        arr, swap_buf, main_is_swap, ptx + div * 14, ptx + div * 13, ptx + div * 15
    )
    return median_of_five(arr, swap_buf, main_is_swap, v2, v0, v1, v3, v4)


def flux_partition(arr, swap_buf, main_is_swap, start, nmemb):
    # The core recursive partition. Reads "main" (the live array if not main_is_swap, else
    # swap_buf) left to right starting at 0 if main_is_swap else start, picks a pivot via
    # median_of_nine/median_of_fifteen, then for every element unconditionally writes it to BOTH
    # the array (at the forward cursor pta) and swap_buf (at the forward cursor pts) -- but only
    # advances whichever cursor is that element's real destination (value > piv -> swap_buf/pts,
    # else array/pta). This is fluxsort's branchless partition trick: the "wrong" write for an
    # element is simply overwritten later by the next element that really belongs at that slot,
    # so no conditional/branch is needed to pick a destination up front. Recurses into whichever
    # side still needs it (skipping straight to quad_sort_range_using once a side is small or
    # skewed enough), high side first.
    ptx_base = 0 if main_is_swap else start
    if nmemb > 1024:
        median_index = median_of_fifteen(arr, swap_buf, main_is_swap, ptx_base, nmemb)
    else:
        median_index = median_of_nine(arr, swap_buf, main_is_swap, ptx_base, nmemb)
    piv = swap_buf[median_index] if main_is_swap else arr[median_index]
    ...
    while ptx < pte:
        value = swap_buf[ptx] if main_is_swap else arr[ptx]
        val = 1 if value > piv else 0
    ...
    if a_size <= s_size // 16 or s_size <= FLUX_OUT:
        for i in range(s_size):
            arr[pta + i] = swap_buf[i]
