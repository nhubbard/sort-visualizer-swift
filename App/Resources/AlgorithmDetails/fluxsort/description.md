...
range does not force a bad pivot on every range, which is what makes the algorithm adaptive to the
data it is sorting rather than to its position in the array. The range is then scanned
