...
fun quadSortRange(arr: Array<Int>, start: Int, length: Int) {
  if (length < 16) {
    ...
    if (quadSwap(arr, start, length) == 0) {
      val buffer = Array(length / 2) { 0 }
      quadMerge(arr, buffer, start, length, 16)
    }
  }
}

fun quadSortRangeUsing(arr: Array<Int>, swapBuf: Array<Int>, start: Int, length: Int) {
  if (length < 16) {
    tailSwap(arr, start, length)
  } else if (length < 256) {
    if (quadSwap(arr, start, length) == 0) {
...
const val FLUX_OUT = 24

fun fluxAnalyze(arr: Array<Int>, nmemb: Int): Boolean {
  var balance = 0
  var pta = 0
  var cnt = nmemb
  while (true) {
...
fun mainGT(arr: Array<Int>, swapBuf: Array<Int>, mainIsSwap: Boolean, a: Int, b: Int): Int {
  if (mainIsSwap) {
    return if (swapBuf[a] > swapBuf[b]) 1 else 0
  }
  return if (arr[a] > arr[b]) 1 else 0
}

fun medianOfThree(
  arr: Array<Int>, swapBuf: Array<Int>, mainIsSwap: Boolean, v0: Int, v1: Int, v2: Int,
): Int {
  var value = mainGT(arr, swapBuf, mainIsSwap, v0, v1)
  var t0 = value
  var t1 = value xor 1

  value = mainGT(arr, swapBuf, mainIsSwap, v0, v2)
  t0 += value
  if (t0 == 1) {
    return v0
  }

  value = mainGT(arr, swapBuf, mainIsSwap, v1, v2)
  t1 += value
  return if (t1 == 1) v1 else v2
}

fun medianOfFive(
  arr: Array<Int>, swapBuf: Array<Int>, mainIsSwap: Boolean,
  ...
  value = mainGT(arr, swapBuf, mainIsSwap, v0, v3)
  t0 += value
  var t3 = value xor 1

  value = mainGT(arr, swapBuf, mainIsSwap, v0, v4)
  t0 += value
  ...
  value = mainGT(arr, swapBuf, mainIsSwap, v1, v3)
  t1 += value
  t3 += value xor 1

  value = mainGT(arr, swapBuf, mainIsSwap, v1, v4)
  t1 += value
  ...
  value = mainGT(arr, swapBuf, mainIsSwap, v2, v3)
  t2 += value
  t3 += value xor 1

  value = mainGT(arr, swapBuf, mainIsSwap, v2, v4)
  t2 += value
  ...
  value = mainGT(arr, swapBuf, mainIsSwap, v3, v4)
  t3 += value

  return if (t3 == 2) v3 else v4
}

fun medianOfNine(arr: Array<Int>, swapBuf: Array<Int>, mainIsSwap: Boolean, ptx: Int, nmemb: Int): Int {
  val div = nmemb / 16
  val v0 = medianOfThree(arr, swapBuf, mainIsSwap, ptx + div * 2, ptx + div * 1, ptx + div * 4)
  val v1 = medianOfThree(arr, swapBuf, mainIsSwap, ptx + div * 8, ptx + div * 6, ptx + div * 10)
  val v2 = medianOfThree(arr, swapBuf, mainIsSwap, ptx + div * 14, ptx + div * 12, ptx + div * 15)
  return medianOfThree(arr, swapBuf, mainIsSwap, v0, v1, v2)
}

fun medianOfFifteen(arr: Array<Int>, swapBuf: Array<Int>, mainIsSwap: Boolean, ptx: Int, nmemb: Int): Int {
  val div = nmemb / 16
  val v0 = medianOfThree(arr, swapBuf, mainIsSwap, ptx + div * 2, ptx + div * 1, ptx + div * 3)
  val v1 = medianOfThree(arr, swapBuf, mainIsSwap, ptx + div * 5, ptx + div * 4, ptx + div * 6)
  val v2 = medianOfThree(arr, swapBuf, mainIsSwap, ptx + div * 8, ptx + div * 7, ptx + div * 9)
  val v3 = medianOfThree(arr, swapBuf, mainIsSwap, ptx + div * 11, ptx + div * 10, ptx + div * 12)
  val v4 = medianOfThree(arr, swapBuf, mainIsSwap, ptx + div * 14, ptx + div * 13, ptx + div * 15)
  return medianOfFive(arr, swapBuf, mainIsSwap, v2, v0, v1, v3, v4)
}

fun fluxPartition(arr: Array<Int>, swapBuf: Array<Int>, mainIsSwap: Boolean, start: Int, nmemb: Int) {
  val ptxBase = if (mainIsSwap) 0 else start
  val medianIndex = if (nmemb > 1024)
    medianOfFifteen(arr, swapBuf, mainIsSwap, ptxBase, nmemb)
  else
    medianOfNine(arr, swapBuf, mainIsSwap, ptxBase, nmemb)
  val piv = if (mainIsSwap) swapBuf[medianIndex] else arr[medianIndex]

  val pte = ptxBase + nmemb
  var pta = start
  var pts = 0
  var ptx = ptxBase

  while (ptx < pte) {
    val value = if (mainIsSwap) swapBuf[ptx] else arr[ptx]
    val v = if (value > piv) 1 else 0
