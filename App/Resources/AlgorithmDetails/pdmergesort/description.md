...
array in half regardless of its contents. Input such as nearly sorted logs or concatenated sorted
batches may contain existing order. A sort that recognizes and reuses that order can finish in a single linear
...
descending, it is reversed in place before moving on. This is an O(k) operation for a run of length k that
...
however many runs the input contains instead of always starting from one run per element.
