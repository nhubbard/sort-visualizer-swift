...
comparisons on favorable input and in O(n log² n) on average, the extra log n factor coming from how many passes a
...
plain recursive Circle Sort's own O(n log n) and O(n log² n) figures closely. Once the budget is exhausted,
though, the binary insertion sort fallback takes over, and its own shifting step degrades to O(n²) on an input that
