...
Because it is built from a fixed schedule of pairwise comparators rather than adapting to the data, the recursive
bitonic sort performs O(log²n) sequential comparison stages regardless of the values being sorted. It is not
a [stable sort](https://en.wikipedia.org/wiki/Sorting_algorithm#Stability): elements can cross paths through comparators
a long distance apart in the array, so equal elements are not guaranteed to keep their original relative order.
