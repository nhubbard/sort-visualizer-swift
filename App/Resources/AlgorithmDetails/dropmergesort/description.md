...
example. The algorithm is optimized for nearly ordered data and falls back to a general-purpose
sorting method when that assumption does not hold.
...
time something is out of place. An element is kept if it is at least as large as the last element
kept; anything smaller gets set aside instead. Because dropping an element costs one write and
no comparisons against everything after it, this pass has low cost for input that is mostly in
order.
...
Once the left-to-right pass finishes (assuming it did not give up early), the dropped list, usually
...
scales with how much of the input is out of order, not with its size alone: a large but
nearly-sorted array can require substantially less work than an equally large shuffled array.
