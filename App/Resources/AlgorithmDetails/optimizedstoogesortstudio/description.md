...
recursive structure to bring the exponent down from O(n²·⁷¹) to a genuine O(n²) worst case,
with O(n) best-case behavior on already-nearly-sorted data, a real complexity improvement, not
just a constant-factor tweak. It is credited to EilrahcF's original concept, aphitorite's
...
split, and tracks whether either partition changed anything. That change-tracking is what
...
perform unconditionally, since there is nothing left to settle.
