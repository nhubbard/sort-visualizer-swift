...
right, that is a sign the left element belongs before every right-run element examined so far, so a
...
by how far a push has to shift elements: an input that is already sorted never triggers a push with
