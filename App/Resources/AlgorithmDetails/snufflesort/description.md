...
Snuffle Sort is a recursive sorting algorithm belonging to ArrayV's "Exchange Sorts" family. Given a range of the array,
...
recursive setup around that one step.

Once a range holds three or more elements, Snuffle Sort re-applies itself to two halves of that same range rather than
moving on. Unlike a typical divide-and-conquer split, the two halves are not disjoint: both the "left" half and the "
right" half include the range's own midpoint, so the element at the midpoint is reprocessed by both recursive calls.
...
landing in `n^log n` territory. ArrayV itself flags Snuffle Sort as deliberately, unreasonably slow and caps the array
