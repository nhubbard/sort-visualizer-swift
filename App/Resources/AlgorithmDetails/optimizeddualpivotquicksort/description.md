...
comes back from partitioning unusually large, and the two pivots turned out to be different values, the algorithm pauses before recursing into it and runs one more scan that moves
...
sorted order those regions already have. Only the narrower remainder, elements strictly
