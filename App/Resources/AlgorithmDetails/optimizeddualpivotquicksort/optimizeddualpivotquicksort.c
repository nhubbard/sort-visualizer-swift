...
#define EQUAL_ELEMENTS_MIN_FRACTION 4
 ...
 * than/greater than a pivot". Writes back, through outLow/outHigh, the
 * inclusive bounds of what's left strictly between the two pivots. */
void movePivotDuplicatesOut(int arr[], int low, int high, int pivot1,
                            int pivot2, int *outLow, int *outHigh) {
  int writeLow = low;
  int read = low;
  int writeHigh = high;
  while (read <= writeHigh) {
    if (arr[read] == pivot1) {
      swapInts(&arr[read], &arr[writeLow]);
      writeLow++;
      read++;
    } else if (arr[read] == pivot2) {
      swapInts(&arr[read], &arr[writeHigh]);
      writeHigh--;
    } else {
      read++;
    }
...
void optimizedDualPivotQuickSort(int arr[], int low, int high) {
  ...
  if (arr[pivot1Index] > arr[pivot2Index]) {
    swapInts(&arr[pivot1Index], &arr[pivot2Index]);
  }
  swapInts(&arr[low], &arr[pivot1Index]);
  swapInts(&arr[high], &arr[pivot2Index]);
  int pivot1 = arr[low];
  int pivot2 = arr[high];
  ...
  while (k <= great) {
    if (arr[k] < pivot1) {
      swapInts(&arr[k], &arr[less]);
      less++;
    } else if (arr[k] > pivot2) {
      while (k < great && arr[great] > pivot2) {
        great--;
      }
      swapInts(&arr[k], &arr[great]);
      great--;
      if (arr[k] < pivot1) {
        swapInts(&arr[k], &arr[less]);
        less++;
  ...
  less--;
  great++;
  swapInts(&arr[low], &arr[less]);
  swapInts(&arr[high], &arr[great]);
