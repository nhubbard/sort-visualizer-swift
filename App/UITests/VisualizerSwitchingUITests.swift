...
final class VisualizerSwitchingUITests: XCTestCase {
  ...
  func testSwitchingVisualizerMidSortDoesNotDisruptTheRunningSession() throws {
    ...
    XCTAssertTrue(
      canvas.waitForExistence(timeout: 5), "canvas disappeared after switching visualizers mid-sort"
    )
    ...
    XCTAssertEqual(
      result, .completed, "sort never reached a terminal state after switching visualizers mid-run")
    XCTAssertEqual(
