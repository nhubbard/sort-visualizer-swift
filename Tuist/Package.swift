...
let packageSettings = PackageSettings(
    productTypes: [
        ...
        // Module Map" Run Script phase for dynamic C-target frameworks whose plain `cp` isn't
        // declared as a sandboxed output — with ENABLE_USER_SCRIPT_SANDBOXING on, Mac Catalyst
        // builds fail with "Permission denied" copying module.modulemap. Leaving both static keeps
        // the build working on every destination; the multiple-targets warning for these two is an
        // accepted tradeoff (see Project.swift generation notes).
    ]
)
#endif
...
let package = Package(
    ...
    dependencies: [
        ...
        // AudioKit/AudioKitUI/AudioKitEX/SoundpipeAudioKit intentionally omitted as of the ToneKit
        // migration — AudioEngineKit's whole audio stack is now Modules/ToneKit, a local target
        // with no external package dependency at all (see its NOTICE.md). This retires a whole
        // class of recurring build-graph pain: pinned tags to dodge a main-branch crash, a
        // Soundpipe-static-library workaround for a module-map cycle, and an AudioKitEX dependency
        // swap already landed once before this.
    ]
)
