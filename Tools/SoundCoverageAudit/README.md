...
Set `SORT_TAPE_EXPORT_DIR` (e.g. `/tmp/sort-tape-audit`) as an environment variable on the app's
run scheme (Xcode → Edit Scheme → Run → Arguments → Environment Variables), then run a full
Showcase pass — the in-app Showcase button, or the "Run Showcase" Shortcut
(`RunShowcaseIntent`). `SortSession.exportTapeForAuditIfRequested`
