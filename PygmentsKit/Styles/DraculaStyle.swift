...
class DraculaStyle: BaseStyle {
  ...
  func getStyle(tokenType: Token.TokenType) -> TextFormat {
    return TextFormat.getBuilder(bg: backgroundColor).then { it in
      switch tokenType {
        case .whitespace, .generic, .genericError, .genericPrompt, .genericStrong, .genericTraceback, .error, .literal, .literalDate, .name, .nameBuiltinPseudo, .nameConstant, .nameDecorator, .nameEntity, .nameException, .nameNamespace, .nameOther, .other, .punctuation, .text:
          it.fg(0xf8f8f2)
        case .genericEmph:
          it.fg(0xf8f8f2).underline()
        case .comment, .commentHashbang, .commentMultiline, .commentSingle, .commentSpecial:
          it.fg(0x6272a4)
        case .commentPreproc, .keyword, .keywordConstant, .keywordNamespace, .keywordPseudo, .keywordReserved, .nameTag, .operator, .operatorWord:
          it.fg(0xff79c6)
        case .genericDeleted:
          it.fg(0x8b080b)
        ...
        case .genericOutput:
          it.fg(0x44475a)
        case .keywordDeclaration, .nameBuiltin, .nameLabel, .nameVariable, .nameVariableClass, .nameVariableGlobal, .nameVariableInstance:
        ...
        case .number, .numberBin, .numberFloat, .numberHex, .numberInteger, .numberIntegerLong, .numberOct:
          it.fg(0xffb86c)
        case .string, .stringBacktick, .stringChar, .stringDoc, .stringDouble, .stringEscape, .stringHeredoc, .stringInterpol, .stringOther, .stringRegex, .stringSingle, .stringSymbol:
          it.fg(0xbd93f9)
        default:
          it.fg(0xffffff)
      }
