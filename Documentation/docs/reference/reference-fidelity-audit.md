...
complexity review of their ten reference sources. Four non-Bogo algorithms below were
verified and corrected; 20 have confirmed differences listed below; the other 145 have no
...
| ShellSort | Uses a fixed Ciura-like gap sequence ending in 1 | All ten references repeatedly halve the gap from `n/2` | Different gap schedule and comparisons; no overall asymptotic difference established |
| RotateLSDRadixSort | Uses base 4 for digit passes and in-place digit merges | All ten references use base 10 | Different digit-pass count and merge partitions; no overall asymptotic difference established |
...
| StablePermutationSort | Tests sortedness with a linear adjacent-value scan at each permutation leaf | Python uses `arr == sorted(arr)` at each leaf; the other nine samples use a linear check | Python adds `O(n log n)` sorting and allocation to each factorial-search leaf |
...
arrays are needed to validate the 20 findings and look for additional edge-case discrepancies.
