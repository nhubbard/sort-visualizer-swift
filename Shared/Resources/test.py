...
def test_c(filename: str) -> bool:
  ...
  try:
    compiler = subprocess.run(" ".join(["clang", "-o", "\"" + outfile + "\"", "\"" + filename + "\""]), shell=True, check=True, capture_output=True)
    compile_stdout = compiler.stdout.decode("utf-8")
    if compile_stdout != "":
      logger.debug(f"Compiler stdout:\n{compile_stdout}")
  except subprocess.CalledProcessError as e:
    logger.error(f"Compilation of {filename} failed! See next entry for error message.")
    logger.error(e.output.decode("utf-8").strip())
    sys.exit(1)
  logger.debug(f"Running ./{outfile}")
  output = subprocess.run([f"./{outfile}"], shell=True, check=True, capture_output=True).stdout.decode("utf-8").strip()
  if standard_expected != output:
    logger.error(f"{filename}: Failed! Expected {standard_expected}, found {output}")
...
def test_cpp(filename: str) -> bool:
  ...
  logger.debug(f"Compiling {filename} into {outfile}")
  try:
    compiler = subprocess.run(" ".join(["clang++", "-o", f"\"{outfile}\"", f"\"{filename}\""]), shell=True, check=True, capture_output=True)
    compile_stdout = compiler.stdout.decode("utf-8")
    if compile_stdout != "":
      logger.debug(f"Compiler stdout:\n{compile_stdout}")
  except subprocess.CalledProcessError as e:
...
def test_cs(filename: str) -> bool:
  ...
  logger.debug(f"Compiling {filename} into {outfile}")
  try:
    subprocess.run(" ".join(["csc", f"-out:\"{outfile}\"", f"\"{filename}\""]), shell=True, check=True, capture_output=True)
  except subprocess.CalledProcessError as e:
    logger.error(f"Compilation of {filename} failed! See next entry for error message.")
    logger.error(e.output.decode("utf-8").strip())
    sys.exit(1)
  logger.debug(f"Running ./{outfile}")
  output = subprocess.run(" ".join(["mono", outfile]), shell=True, check=True, capture_output=True).stdout.decode("utf-8").strip()
  if standard_expected != output:
    logger.error(f"{filename}: Failed! Expected {standard_expected}, found {output}")
    if os.path.exists(outfile):
      logger.debug(f"Deleting {outfile}")
...
def test_go(filename: str) -> bool:
  ...
  try:
    output = subprocess.run(" ".join(["go", "run", filename]), shell=True, check=True, capture_output=True).stdout.decode("utf-8").strip()
...
def test_java(filename: str) -> bool:
  ...
  logger.debug(f"Compiling {filename} into {outfile}")
  try:
    compiler = subprocess.run(" ".join(["javac", filename]), shell=True, check=True, capture_output=True)
    compile_stdout = compiler.stdout.decode("utf-8")
    if compile_stdout != "":
      logger.debug(f"Compiler stdout:\n{compile_stdout}")
  except subprocess.CalledProcessError as e:
    ...
    sys.exit(1)
  logger.debug(f"Running ./{outfile}")
  output = subprocess.run(" ".join(["java", "-cp", classpath, filename.split("/")[1].replace(".java", "")]), shell=True, check=True, capture_output=True).stdout.decode("utf-8").strip()
  if standard_expected != output:
    logger.error(f"{filename}: Failed! Expected {standard_expected}, found {output}")
    if os.path.exists(outfile):
      logger.debug(f"Deleting {outfile}")
...
def test_js(filename: str) -> bool:
  logger.debug(f"Running {filename}")
  try:
    output = subprocess.run(" ".join(["node", filename]), shell=True, check=True, capture_output=True).stdout.decode("utf-8").strip()
    if standard_expected != output:
      logger.error(f"{filename}: Failed! Expected {standard_expected}, found {output}")
      return False
    else:
...
def test_kt(filename: str) -> bool:
  ...
  logger.debug(f"Compiling {filename} into {outfile}")
  try:
    compiler = subprocess.run(" ".join(["kotlinc", filename]), shell=True, check=True, capture_output=True)
    compile_stdout = compiler.stdout.decode("utf-8")
    if compile_stdout != "":
      logger.debug(f"Compiler stdout:\n{compile_stdout}")
  except subprocess.CalledProcessError as e:
    ...
    sys.exit(1)
  logger.debug(f"Running ./{outfile}")
  output = subprocess.run(" ".join(["kotlin", outfile]), shell=True, check=True, capture_output=True).stdout.decode("utf-8").strip()
  if standard_expected != output:
    logger.error(f"{filename}: Failed! Expected {standard_expected}, found {output}")
...
def test_py(filename: str) -> bool:
  logger.debug(f"Running {filename}")
  try:
    output = subprocess.run(" ".join(["python3", filename]), shell=True, check=True, capture_output=True).stdout.decode("utf-8").strip()
    if standard_expected != output:
      logger.error(f"{filename}: Failed! Expected {standard_expected}, found {output}")
      return False
    else:
...
def test_rb(filename: str) -> bool:
  logger.debug(f"Running {filename}")
  try:
    output = subprocess.run(" ".join(["/opt/homebrew/opt/ruby/bin/ruby", filename]), shell=True, check=True, capture_output=True).stdout.decode("utf-8").strip()
    if standard_expected != output:
      logger.error(f"{filename}: Failed! Expected {standard_expected}, found {output}")
      return False
    else:
...
def test_swift(filename: str) -> bool:
  ...
  logger.debug(f"Compiling {filename} into {outfile}")
  try:
    compiler = subprocess.run(" ".join(["swiftc", filename]), shell=True, check=True, capture_output=True)
    compile_stdout = compiler.stdout.decode("utf-8")
    if compile_stdout != "":
      logger.debug(f"Compiler stdout:\n{compile_stdout}")
  except subprocess.CalledProcessError as e:
