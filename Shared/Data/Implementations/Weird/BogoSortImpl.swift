...
extension SortViewModel {
  ...
  func bogoSort() async -> SortStatus {
    ...
    // Don't use uniquePermutations(); we're guaranteed to not have any duplicate values in the array, and the unique
    // check slows down the operation from O(1) to O(n).
    ...
    for p in permutations {
      ...
      await data.indices.concurrentForEach(withPriority: .high) { [self] i in
        // It may look weird that I'm wrapping a "non-null" value in an Optional result, but it stops the compiler
        // from complaining about it, so I call that a win. The result is actually nullable, since we're attempting
        // to get an index from an array that might be out of bounds.
        guard
